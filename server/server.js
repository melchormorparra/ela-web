...
app.post('/api/login', (req, res) => {
    ...
    if (password === data.admin.password) {
        res.json({ success: true, token: 'admin-token', user: { id: 0, name: 'Admin', email: data.admin.email, isAdmin: true } });
    } else {
...
app.get('/api/products', (req, res) => {
    const data = readData();
    res.json(data.products.filter(p => p.active));
});

app.get('/api/blog', (req, res) => {
    const data = readData();
    res.json(data.blogPosts.filter(p => p.active));
});
